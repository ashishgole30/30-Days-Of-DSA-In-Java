public class Number_of_Ways_to_Split_Array {
    public static void main(String[] args) {
        ...
        int numofvalidparis = 0;

        for (int i = 0; i < n - 1; i++) {
            int leftsum = prefixarray[i];
            int rightsum = prefixarray[n - 1] - prefixarray[i];
            if (leftsum >= rightsum) {
                numofvalidparis++;
            }
        }
        System.out.println(numofvalidparis);
    }
}
/*
 * thinking process:
 * 1. we have input int array nums
 * 2.find its length store it n
 * 3.create prefixarray of length n
 * 4.prefix[0] = nums[0]
 * 5.build prefix array using for loop
 * i = 1 ; i<n ;i++
 * prefix[i] = prefix[i-1] + nums[i]
 * 6.now initially numofvalid pairs = 0;
 * 7. now for i=0 ; i<n=1 ; i++
 * note : i<n-1 cause right part of i cannot be empty , must contain atleast 1
 * element
 * calculate leftsum using prefix formula
 * note( split happens after index i, so left side must include element
 * at index i — and prefix[i] already includes it.
 * prefix[i-1] would exclude the current element, which is wrong.)
 * calculate rightsum using prefix formula
 * if leftsum>=rightsum
 * increment numofvalidpars by 1
 * 8.return numofvalid pairs
 */
